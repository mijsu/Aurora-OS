...
export function migrateFileSystem(storedFS: FileNode, initialFS: FileNode): FileNode {
    ...
    // Explicit Healing: Sync sizes for mock files that might have been lost in legacy snapshots
    function healFileSizes(node: FileNode, fresh: FileNode) {
        if (node.type === fresh.type && node.name === fresh.name) {
            if (node.type === 'file' && fresh.size && !node.size) {
                console.log(`Migration: Restoring size for ${node.name}: ${fresh.size}B`);
                node.size = fresh.size;
            }
        }

        if (fresh.children && node.children) {
            for (const freshChild of fresh.children) {
                const storedChild = node.children.find(c => c.name === freshChild.name);
                if (storedChild) {
                    healFileSizes(storedChild, freshChild);
                }
            }
        }
    }

    healFileSizes(storedFS, initialFS);
