...
import { ChevronLeft, ChevronRight, FolderOpen, FileText, Download, HardDrive, Search, Grid3x3, List, Monitor, Music, Image, Trash, Trash2, Settings, Home, Clipboard, Info, Scissors, Copy, Upload, Film } from 'lucide-react';
...
import { getCategoryFolderForFile } from '@/utils/fileAssociations';
...
export function FileManager({ id, initialPath, onOpenApp, owner }: { id: string; initialPath?: string; onOpenApp?: (id: string, args?: any, owner?: string) => void, owner?: string }) {
  ...
  const { listDirectory, moveNodeById, getNodeAtPath, moveToTrash, resolvePath, users, createDirectory, clipboard, copyNodes, cutNodes, pasteNodes, createFile, deleteNode } = useFileSystem();
  ...
  const handleOpenItem = useCallback((item: FileNode, path: string) => {
    if (item.type === 'directory') {
      ...
      const isDoc = /\.(pdf|docx)$/i.test(item.name);
      ...
      if (isMusic) {
        ...
        // Check if video-player app is installed by checking /usr/bin
        const videoPlayerBinary = getNodeAtPath('/usr/bin/video-player', activeUser);
        if (videoPlayerBinary) {
          const fullPath = resolvePath(path, activeUser);
          if (onOpenApp) onOpenApp('video-player', { path: fullPath, timestamp: Date.now() }, activeUser);
        } else {
          notify.system('error', 'Finder', t('fileManager.toasts.videoPlayerNotInstalled'), t('notifications.subtitles.appMissing'));
        }
      } else if (isDoc) {
        // Check if document-viewer app is installed by checking /usr/bin
        const docViewerBinary = getNodeAtPath('/usr/bin/document-viewer', activeUser);
        if (docViewerBinary) {
          const fullPath = resolvePath(path, activeUser);
          if (onOpenApp) onOpenApp('document-viewer', { path: fullPath, timestamp: Date.now() }, activeUser);
        } else {
          notify.system('error', 'Finder', t('fileManager.toasts.documentViewerNotInstalled'), t('notifications.subtitles.appMissing'));
        }
      } else {
  ...
  // File Upload Handler with Auto-Categorization (Move instead of Copy)
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const readFileWithProgress = (file: File, isText: boolean, onProgress: (progress: number) => void): Promise<string> => {
      return new Promise((resolve, reject) => {
        ...
        reader.onprogress = (event) => {
          if (event.lengthComputable) {
            ...
            onProgress(Math.min(progress, 85)); // Cap at 85% for reading phase
          }
        };

        reader.onload = (e) => {
          onProgress(90); // Bump to 90% after reading complete
    ...
    for (const file of Array.from(files)) {
      // Yield to event loop every iteration to keep UI responsive
      ...
      try {
        ...
        const checkExists = (n: string, path: string) => {
          const dirNode = getNodeAtPath(path, activeUser);
          if (!dirNode || dirNode.type !== 'directory' || !dirNode.children) return false;
          return dirNode.children.some(i => i.name === n);
        };

        // Determine permissions based on user
        const permissions = activeUser === 'root' ? '-rwxrwxrwx' : '-rw-r--r--';

        // Determine if file should use native storage
        const isBinaryMedia = file.type.startsWith('image/') || file.type.startsWith('video/') || file.type.startsWith('audio/');
        const isSupportedCapacitor = fileStorage.isSupportedCapacitor();
        const shouldStoreNatively = fileStorage.shouldUseNativeStorage(file.size) || (isSupportedCapacitor && isBinaryMedia);

        // Show progress for files > 2MB
        const showProgress = file.size > 2 * 1024 * 1024;
        const progressTracker = showProgress ? notify.uploadProgress(originalName, formatFileSize(file.size)) : null;

        // Update initial progress
        if (progressTracker) progressTracker.updateProgress(5);

        // Get the category folder for this file type
        const categoryFolder = getCategoryFolderForFile(originalName, homePath);
        const categoryNode = categoryFolder ? getNodeAtPath(categoryFolder, activeUser) : null;
        
        // IMPORTANT: Keep file in CURRENT location (where user uploaded), then copy to category + Downloads
        const targetPath = currentPath;
        const organizationFolders: string[] = [];

        // Check if file exists in current location and rename if needed
        let finalName = originalName;
        let fileCounter = 1;
        while (checkExists(finalName, targetPath)) {
          finalName = `${baseName} (${fileCounter}).${ext}`;
          fileCounter++;
        }

        // Create file in CURRENT location (where user uploaded)
        let success = false;
        let fileUri: string | undefined;
        let fileSize: number | undefined;
        
        if (shouldStoreNatively) {
          if (progressTracker) progressTracker.updateProgress(10);
          // For native files, show smooth incremental progress
          let currentProgress = 10;
          const progressInterval = setInterval(() => {
            currentProgress += Math.random() * 3; // Increment by 0-3% each interval
            currentProgress = Math.min(currentProgress, 85); // Cap at 85%
            if (progressTracker) progressTracker.updateProgress(Math.floor(currentProgress));
          }, 200);
          ...
          try {
            ...
            if (progressTracker) progressTracker.updateProgress(95);
            success = createFile(targetPath, finalName, '', activeUser, permissions, fileUri, fileSize);
            if (progressTracker) progressTracker.updateProgress(100);
          } catch (error) {
            clearInterval(progressInterval);
            throw error;
          }
        } else {
          // For text files, read with real progress tracking
          const isText = file.type.startsWith('text/') || /\.(txt|md|json|js|ts|css|html)$/i.test(file.name);
          const content = await readFileWithProgress(file, isText, (progress) => {
            if (progressTracker) progressTracker.updateProgress(progress);
          });
          
          if (progressTracker) progressTracker.updateProgress(95);
          success = createFile(targetPath, finalName, content, activeUser, permissions);
          if (progressTracker) progressTracker.updateProgress(100);
        }

        if (success) {
          // Copy to category folder if applicable
          if (categoryFolder && categoryNode && categoryNode.type === 'directory') {
            let categoryFileName = finalName;
            let categoryCounter = 1;
            while (checkExists(categoryFileName, categoryFolder)) {
              categoryFileName = `${baseName} (${categoryCounter}).${ext}`;
              categoryCounter++;
            }

            try {
              if (shouldStoreNatively && fileUri) {
                createFile(categoryFolder, categoryFileName, '', activeUser, permissions, fileUri, fileSize);
              } else if (!shouldStoreNatively) {
                const isText = file.type.startsWith('text/') || /\.(txt|md|json|js|ts|css|html)$/i.test(file.name);
                const content = await readFileWithProgress(file, isText, () => {
                  // Silent progress for category copy
                });
                createFile(categoryFolder, categoryFileName, content, activeUser, permissions);
              }
              const folderName = categoryFolder.split('/').pop() || categoryFolder;
              organizationFolders.push(folderName);
            } catch (error) {
              console.warn('Failed to copy to category folder:', error);
            }
          }

          // Copy to Downloads if not already there
          if (currentPath !== downloadsPath && getNodeAtPath(downloadsPath, activeUser)) {
            let downloadsFileName = finalName;
            let downloadsCounter = 1;
            while (checkExists(downloadsFileName, downloadsPath)) {
              downloadsFileName = `${baseName} (${downloadsCounter}).${ext}`;
              downloadsCounter++;
            }

            try {
              if (shouldStoreNatively && fileUri) {
                createFile(downloadsPath, downloadsFileName, '', activeUser, permissions, fileUri, fileSize);
              } else if (!shouldStoreNatively) {
                const isText = file.type.startsWith('text/') || /\.(txt|md|json|js|ts|css|html)$/i.test(file.name);
                const content = await readFileWithProgress(file, isText, () => {
                  // Silent progress for Downloads copy
                });
                createFile(downloadsPath, downloadsFileName, content, activeUser, permissions);
              }
              organizationFolders.push('Downloads');
            } catch (downloadError) {
              console.warn('Failed to copy to Downloads:', downloadError);
            }
          }
          ...
          let categorizedMessage = t('fileManager.toasts.uploadedFile', { name: finalName });
          if (organizationFolders.length > 0) {
            categorizedMessage = `${t('fileManager.toasts.uploadedFile', { name: finalName })} - ${t('fileManager.toasts.organizedTo')} ${organizationFolders.join(', ')}`;
          }

          notify.system('success', 'Finder', categorizedMessage, t('notifications.subtitles.uploadComplete'));
        } else {
          if (progressTracker) progressTracker.complete();
          notify.system('error', 'Finder', t('fileManager.toasts.uploadFailed', { name: originalName }), t('notifications.subtitles.failed'));
        }
      } catch (error) {
        console.error('Upload failed:', error);
        notify.system('error', 'Finder', t('fileManager.toasts.uploadFailed', { name: file.name }), t('notifications.subtitles.failed'));
      }
    }
  ...
  // Cascade delete - delete file from all organized locations
  const cascadeDelete = (filePath: string, fileName: string) => {
    try {
      // Move primary file to trash from current location
      moveToTrash(filePath, activeUser);

      // Check if this is a categorized file and delete duplicates from other locations
      const categoryFolder = getCategoryFolderForFile(fileName, homePath);
      const downloadsPath = `${homePath}/Downloads`;

      // Permanently delete duplicate from category folder if applicable
      if (categoryFolder) {
        const categoryFilePath = categoryFolder === '/' ? `/${fileName}` : `${categoryFolder}/${fileName}`;
        const categoryNode = getNodeAtPath(categoryFilePath, activeUser);
        if (categoryNode) {
          deleteNode(categoryFilePath, activeUser);
        }
      }

      // Permanently delete duplicate from Downloads if not current location
      if (!filePath.startsWith(downloadsPath)) {
        const downloadsFilePath = downloadsPath === '/' ? `/${fileName}` : `${downloadsPath}/${fileName}`;
        const downloadsNode = getNodeAtPath(downloadsFilePath, activeUser);
        if (downloadsNode) {
          deleteNode(downloadsFilePath, activeUser);
        }
      }
    } catch (error) {
  ...
  const fileManagerSidebar = useMemo(() => {
    ...
    // Standard folders that should always be shown
    const standardFolderIds = ['home', 'desktop', 'documents', 'downloads', 'pictures', 'videos', 'music'];

    // Helper to Create Items if they exist
    const createSidebarItem = (id: string, icon: any, label: string, path: string) => {
      let node = getNodeAtPath(path, activeUser);
      
      // If node doesn't exist but it's a standard folder, create it
      if (!node && standardFolderIds.includes(id)) {
        const folderName = path.split('/').pop();
        if (folderName) {
          createDirectory(userHomePath, folderName, activeUser);
          node = getNodeAtPath(path, activeUser);
        }
      }
    ...
    const favoritesToCheck = [
      ...
      { id: 'videos', icon: Film, label: t('fileManager.places.videos'), path: `${userHomePath}/Videos` },
    ...
    <div className="flex items-center w-full gap-2 px-0">
          ...
          onClick={() => {
            if (selectedItems.size > 0) {
              selectedItems.forEach(id => {
                ...
                if (item) {
                  const fullPath = currentPath === '/' ? `/${item.name}` : `${currentPath}/${item.name}`;
                  cascadeDelete(fullPath, item.name);
                }
              });
              setSelectedItems(new Set());
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          className="p-1.5 rounded-md transition-colors text-white/50 hover:bg-white/5 hover:text-white"
