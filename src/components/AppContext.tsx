...
import { getDeviceTotalRAM } from '@/utils/deviceInfo';
...
interface AppContextType {
  ...
  // Picture-in-Picture Video State (Global)
  floatingVideoData: FloatingVideoData | null;
  setFloatingVideoData: (data: FloatingVideoData | null) => void;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Picture-in-Picture Video State (Global - persists across window closes)
  const [floatingVideoData, setFloatingVideoData] = useState<FloatingVideoData | null>(null);
  ...
  // Detect actual device RAM on app initialization
  useEffect(() => {
    const detectDeviceMemory = async () => {
      if (totalMemoryGB === DEFAULT_SYSTEM_MEMORY_GB) {
        // Only detect if still at default value
        const detectedMemory = await getDeviceTotalRAM();
        setTotalMemoryGB(detectedMemory);
      }
    };
    detectDeviceMemory();
  }, [totalMemoryGB, setTotalMemoryGB]);
