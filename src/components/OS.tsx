...
import { FloatingVideoPlayer } from '@/components/apps/FloatingVideoPlayer';
...
export default function OS() {
    const { activeUser, floatingVideoData, setFloatingVideoData } = useAppContext();
    const { t } = useI18n();
    ...
    const getAppContent = useCallback((type: string, data?: any, owner?: string): { content: React.ReactNode, title: string } => {
        ...
        switch (type) {
            ...
            case 'video-player':
                title = 'Video Player';
                content = <VideoPlayer owner={owner} initialPath={data?.path} />;
                break;
            case 'pdf-viewer':
            case 'document-viewer':
                title = 'Document-Viewer';
                content = <DocumentViewer owner={owner} initialPath={data?.path} />;
                break;
            default:
    ...
    const handleIconDoubleClick = useCallback((iconId: string) => {
        ...
        if (node?.type === 'directory') {
            ...
            const isPdf = /\.pdf$/i.test(icon.name);
            ...
            } else if (/\.(mp4|webm|mov|avi|mkv)$/i.test(icon.name)) {
                const videoBinary = getNodeAtPath('/usr/bin/video-player');
                if (videoBinary) {
                    openWindow('video-player', { path, timestamp: Date.now() });
                } else {
                    notify.system('error', 'OS', t('os.toasts.videoPlayerNotInstalled'), t('notifications.subtitles.appMissing'));
                }
            } else if (isPdf) {
                const docBinary = getNodeAtPath('/usr/bin/document-viewer');
                if (docBinary) {
                    openWindow('document-viewer', { path, timestamp: Date.now() });
                } else {
                    notify.system('error', 'OS', t('os.toasts.documentViewerNotInstalled'), t('notifications.subtitles.appMissing'));
                }
            }
        }
    ...
    const focusedAppType = useMemo(() => {
        ...
        // Extract app type by removing the timestamp (last numeric segment)
        const parts = focusedWindowId.split('-');
    ...
    return (
        <AppNotificationsProvider onOpenApp={openWindow}>
            <div className="dark h-screen w-screen overflow-hidden bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 relative">
                ...
                {/* Global Picture-in-Picture Floating Video Player */}
                <AnimatePresence>
                    {floatingVideoData && (
                        <FloatingVideoPlayer
                            data={floatingVideoData}
                            onClose={() => setFloatingVideoData(null)}
                            onUpdate={(updates) => {
                                setFloatingVideoData({ ...floatingVideoData, ...updates });
                            }}
                        />
                    )}
                </AnimatePresence>
