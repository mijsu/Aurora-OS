...
import {
    ...
    MessageSquare, Send, Film, Music, Image, FileVideo
} from 'lucide-react';
...
const countNodes = (node: FileNode): { files: number, dirs: number, size: number } => {
    ...
    // Use node.size (native storage) if available, otherwise fallback to content length
    let size = 0;
    if (node.type === 'file') {
...
const FileSystemTree: React.FC<{
    ...
    selectedPath?: string;
}> = ({ node, depth = 0, onSelectFile, selectedPath }) => {
            ...
            >
                {node.type === 'directory' ? (
                    <>
                        ...
                        <Folder className={cn("w-3.5 h-3.5", isSelected ? "text-blue-400" : "text-blue-400/60")} />
                    </>
                ) : (
                    <>
                        <div className="w-3.5" />
                        <FileText className={cn("w-3.5 h-3.5", isSelected ? "text-white/80" : "text-white/30")} />
                    </>
                )}
                <span className="text-xs truncate">{node.name}</span>
                {node.type === 'file' && (
                    <span className="ml-auto text-[10px] text-white/20">
                        {formatBytes(node.size || (node.content ? new Blob([node.content]).size : 0))}
                    </span>
                )}
            </div>
...
export function DevCenter() {
    const { fileSystem, currentUser, healMetadata } = useFileSystem();
    const { activeUser, totalMemoryGB } = useAppContext();
    ...
    const [nativeStats, setNativeStats] = useState<{ count: number, size: number }>({ count: 0, size: 0 });
    const [nativeFiles, setNativeFiles] = useState<Array<{ name: string, path: string, category: string, size: number }>>([]);
    ...
    useEffect(() => {
        ...
        const updateStats = async () => {
            ...
            // Native Storage Stats (if on storage or apps-media tab)
            if ((activeTab === 'storage' || activeTab === 'apps-media') && fileStorage.isSupportedCapacitor()) {
                const stats = await fileStorage.getNativeStorageStats();
                setNativeStats(stats);
                // Also load the file list when on apps-media tab
                if (activeTab === 'apps-media') {
                    const files = await fileStorage.getNativeStorageFiles();
    ...
    const handleHealMetadata = async () => {
        try {
            const result = await healMetadata();
            if (result.fixed > 0) {
                notify.system('success', 'Metadata Healing', `Repaired ${result.fixed} file(s) across ${result.total} checked.`);
            } else {
                notify.system('success', 'Metadata Healing', `All ${result.total} files are already in sync.`);
            }
        } catch (error) {
            console.error('Heal Metadata failed:', error);
            notify.system('error', 'Metadata Healing', 'Healing process failed.');
        }
    };
    ...
    const devSidebar = {
        sections: [
            ...
            },
            {
                ...
                items: [
                    { id: 'apps-messages', label: t('messages.title'), icon: MessageSquare },
                    { id: 'apps-media', label: 'Media Storage', icon: Film },
                ]
            }
        ]
    };
    ...
    const renderContent = ({ width }: { width: number }) => {
        ...
        switch (activeTab) {
            ...
            case 'memory': {
                const totalRam = totalMemoryGB * 1024; // Convert GB to MB
                ...
                return (
                    <div className="h-full flex flex-col p-6">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <span className="text-xs font-mono text-white/50">Total: {totalMemoryGB} GB</span>
                        </div>
                                            ...
                                            <div>
                                                ...
                                                <div className="bg-white/5 rounded-lg p-3 space-y-2 text-xs font-mono">
                                                    <div className="flex justify-between">
                                                        ...
                                                        <span className="text-white">{formatBytes(selectedFileNode.size || (selectedFileNode.content ? new Blob([selectedFileNode.content]).size : 0))}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                    ...
                                                    {selectedFileNode.nativeUri && (
                                                        <div className="flex flex-col gap-1 pt-2 border-t border-white/5">
                                                            <span className="text-blue-400 text-[10px] uppercase font-bold">Native Storage</span>
                                                            <div className="text-[9px] text-white/40 break-all leading-tight">
                                                                {selectedFileNode.nativeUri}
                                                            </div>
                                                        </div>
                                                    )}
            ...
            case 'storage': {
                ...
                return (
                    <div className="h-full flex flex-col p-6">
                        ...
                        {fileStorage.isSupportedCapacitor() && (
                            <div className="mb-8">
                                <h3 className="text-lg text-white mb-4">Native File Storage (Zero-Base64)</h3>
                                <div className="bg-blue-500/10 p-4 rounded-lg border border-blue-500/20 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="text-sm text-blue-300 font-medium">Capacitor Filesystem Usage</div>
                                            <div className="text-xs text-white/40">Stored in /aurora-files/</div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-2xl text-white font-mono">{formatBytes(nativeStats.size)}</div>
                                        <div className="text-xs text-white/30">{nativeStats.count} Binary Objects</div>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="h-full flex flex-col p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl text-white font-medium flex items-center gap-3">
                                ...
                                Media Storage (Zero-Base64)
                            </h2>
                            <div className="flex gap-4 text-xs text-white/40 font-mono">
                                <span>{nativeStats.count} Objects</span>
                                <span>{formatBytes(nativeStats.size)} Total</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-hidden border border-white/10 rounded-lg bg-black/20 flex flex-col">
                            <div className="grid grid-cols-12 gap-2 p-3 border-b border-white/10 text-xs font-medium text-white/40 uppercase tracking-wider bg-white/5">
                                ...
                                <div className="col-span-5">Relative Path</div>
                            ...
                            <div className="overflow-y-auto flex-1">
                                {nativeFiles.length === 0 ? (
                                    <div className="p-12 text-center flex flex-col items-center gap-3">
                                        ...
                                        <div className="text-white/20 text-sm">No native media files found.</div>
                                    </div>
                                ) : (
                                    nativeFiles.map((file, idx) => {
                                        const getCategoryIcon = () => {
                                            if (file.category === 'videos') return <FileVideo className="w-4 h-4 text-blue-400" />;
                                            if (file.category === 'audio') return <Music className="w-4 h-4 text-purple-400" />;
                                            if (file.category === 'images') return <Image className="w-4 h-4 text-green-400" />;
                                            return <FileText className="w-4 h-4 text-white/60" />;
                                        };
                                        
                                        return (
                                            <div key={idx} className="grid grid-cols-12 gap-2 px-3 py-2 border-b border-white/5 hover:bg-white/5 text-sm group">
                                                <div className="col-span-1 flex items-center justify-center">{getCategoryIcon()}</div>
                                                <div className="col-span-5 text-white/80 truncate font-mono text-xs" title={file.path}>{file.name}</div>
                                                <div className="col-span-4 text-white/60 capitalize text-xs">{file.category}</div>
                                                <div className="col-span-2 text-right text-white/50 text-xs font-mono">{formatBytes(file.size)}</div>
                                            </div>
                                        );
                                    })
                                )}
                            ...
                            <div className="p-4 border-t border-white/10 bg-white/2 flex justify-between items-center">
                                <div className="text-[10px] text-white/40 max-w-[70%]">
                                    Files &gt;= 1MB are stored natively to prevent JavaScript heap overflows.
                                </div>
                                <GlassButton
                                    size="sm"
                                    className="gap-1.5"
                                    onClick={handleHealMetadata}
                                >
                                ...
                                <GlassButton
                                    ...
                                    variant="danger"
                                    onClick={async () => {
                                        if (confirm("Clear all native storage? This will break VFS links!")) {
                                            ...
                                            setNativeFiles([]);
                                            notify.system('success', 'Media Storage', 'Native storage cleared');
                                        }
                                    }}
                                >
                                    Force Clear
