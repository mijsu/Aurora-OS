...
export interface Song {
    ...
    album: string;
    duration: string;
    nativeUri?: string; // Add native URI support for streaming
}
...
export function MusicProvider({ children, owner }: { children: React.ReactNode, owner?: string }) {
    const { fileSystem, getNodeAtPath, resolvePath, listDirectory } = useFileSystem();
    ...
    const [playlist, setPlaylistInternal] = useState<Song[]>(() => {
        try {
            ...
            if (saved) {
                ...
                return parsed.map(s => {
                    const node = getNodeAtPath(s.path, activeUser);
                    if (node && node.type === 'file') {
                        const url = fileStorage.getFileUrl(node);
                        if (url && url !== s.url) {
                            return { ...s, url, nativeUri: node.nativeUri };
                        }
                    }
                    return s;
                });
    ...
    }, [getNodeAtPath, activeUser, playSong, setActiveCategory, getSafeMeta]);
        ...
        if (files) {
            ...
            const parsedSongs: Song[] = audioFiles.map((file: any) => {
                ...
                return {
                    ...
                    url: fileStorage.getFileUrl(file),
