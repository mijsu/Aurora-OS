...
import { CORE_APP_IDS, PREINSTALLED_OPTIONAL_APP_IDS } from "../config/appConstants";
...
export interface FileSystemContextType {
  ...
  healMetadata: () => Promise<{ fixed: number; total: number }>;
}
...
export function FileSystemProvider({ children }: { children: ReactNode }) {
  ...
  const [installedApps, setInstalledApps] = useState<Set<string>>(() => {
    ...
    // Default to core apps + pre-installed optional apps
    return new Set([...CORE_APP_IDS, ...PREINSTALLED_OPTIONAL_APP_IDS]);
  });
  ...
  const resetFileSystem = useCallback((silent: boolean = false) => {
    // Reset installed apps to core apps + pre-installed optional apps
    setInstalledApps(new Set([...CORE_APP_IDS, ...PREINSTALLED_OPTIONAL_APP_IDS]));
  ...
  // --- Metadata Healing ---
  const healMetadata = useCallback(async () => {
    try {
      ...
      const traverse = async (node: FileNode) => {
        if (node.type === 'file') {
          ...
          if (node.nativeUri) {
            try {
              const parts = node.nativeUri.split('/');
              ...
              if (node.size !== stat.size) {
                node.size = stat.size;
                fixedCount++;
              }
            } catch (e) {
              console.warn(`Failed to stat native file: ${node.name}`, e);
            }
          }
        }
      ...
      await traverse(newFS);

      if (fixedCount > 0) {
        setFileSystem(newFS);
      }
      return { fixed: fixedCount, total: totalFiles };
    } catch (err) {
      console.error('Heal metadata failed:', err);
