...
export function useFileSystemMutations({
    ...
    const createFile = useCallback((path: string, name: string, content: string = '', asUser?: string, permissions: string = '-rw-r--r--', nativeUri?: string, size?: number): boolean => {
        const resolved = resolvePath(path);
        ...
        const newFile: FileNode = {
            ...
            name,
            type: 'file',
            content: nativeUri ? '' : content, // If we have nativeUri, don't store content
            nativeUri, // Store native file URI for large files
            size: size !== undefined ? size : content.length,
            modified: new Date(),
